...
proc dispatchMetaPacket*(vm: StreamingPreviewVM; jsonBody: string) =
  ...
  ##
  ## RS-M13b: the new ``render-tree`` M sub-kind drives the editor's
  ## DOM-rendering host. The Nim viewmodel doesn't need to cache the
  ## tree (the JS materializer applies it directly to the host
  ## element); we only route through here so unit tests can hook
  ## ``dispatchMetaPacket`` and assert on body content.
  if isElementTreeBody(jsonBody):
    ...
    try:
      ...
      discard
    return
  if isRenderTreeBody(jsonBody):
    # The render-tree body carries presentation (tag / text / style /
    # children) and the JS materializer in ``attachRenderTreeClient``
    # applies it directly to the host DOM subtree. Nothing to mutate
    # on the viewmodel here — element-tree manifest stays
    # authoritative for hit-test bounds.
    discard
...
when defined(js):
  ...
  # ---------------------------------------------------------------------------
  # RS-M13b: render-tree DOM client (GPUI / Freya).
  # ---------------------------------------------------------------------------
  type
    RenderTreeHandle* = ref object
      ## Opaque handle for an attached RS-M13b render-tree DOM client.
      ## Holds the live WebSocket and the host DIV the materialiser
      ## paints DOM into. Mutually exclusive with ``BridgeClientHandle``
      ## and ``TuiTerminalHandle`` on a single binding.
      socket*: JsObject
      host*: Element
      rendererId*: string
      url*: string

  proc attachRenderTreeClient*(vm: StreamingPreviewVM; host: Element;
                               bridgeUrl: string; rendererId: string;
                               onVectorSymbolDblClick: proc(componentPath: string)
                                   = nil;
                               onWsOpen: proc() = nil): RenderTreeHandle =
    ## RS-M13b: open a WS to ``bridgeUrl``, decode the F/M/I packet
    ## stream, and materialise every ``render-tree`` M sub-kind into a
    ## DOM subtree under ``host``. Element-tree M sub-kinds keep
    ## flowing through ``dispatchMetaPacket`` so the editor's overlay
    ## continues to read its bounds from there.
    ##
    ## F packets are silently ignored: the launcher advertises
    ## ``rendererSurface = "tree"`` which means no F packets will
    ## arrive on the wire. The ignore branch keeps the editor
    ## defensive against an older launcher.
    var socket: JsObject
    let dispatchMeta = proc(body: cstring) =
      vm.dispatchMetaPacket($body)
    let onClick = proc(x: int; y: int) =
    ...
    let onDblClick = proc(x: int; y: int) =
      ## M-EVP-11: hit-test the manifest at the dblclick coordinate.
      ## When the resolved entry's kind == "vector-symbol", fire the
      ## editor's vector-editor open path through the supplied
      ## callback. Mirrors the canvas-client dblclick wiring.
      let hit = vm.canvas.elementAt(x, y)
      if hit.isNone:
        return
      let entry = hit.get
      ...
      if onVectorSymbolDblClick != nil:
        onVectorSymbolDblClick(entry.componentPath)
    {.emit: ["""
      (function (host, url, rendererId, dispatchMeta, onClick, onHover, onDblClick, onWsOpen) {
        if (!host || !url) return null;
        // Mark the host element + apply renderer-id class so the
        // matching CSS bundle scopes its selectors here.
        host.setAttribute('data-render-tree-host', rendererId);
        if (!host.classList.contains('render-tree-host--' + rendererId)) {
          host.classList.add('render-tree-host--' + rendererId);
        }
        host.classList.add('render-tree-host');
        var ws = new WebSocket(url);
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('open', function () {
          try { if (onWsOpen) onWsOpen(); } catch (_) {}
        ...
        // Track the latest manifest so callers (and tests) can re-read
        // without diffing the live DOM.
        host.__isonimRenderTreeManifest = null;
        function applyStyle(el, styleMap) {
          if (!styleMap) return;
          for (var k in styleMap) {
            if (!Object.prototype.hasOwnProperty.call(styleMap, k)) continue;
            try { el.style.setProperty(k, styleMap[k]); } catch (_) {}
          }
        }
        function materialiseNode(node) {
          // Tag whitelist: the render-tree carries whatever the
          // renderer's headless tree had. Fall back to 'div' for
          // unknown / Skia-only vocabulary (e.g. Freya's 'rect',
          // 'paragraph', 'label', 'image', 'ScrollView') so the DOM
          // stays well-formed regardless of source.
          var tag = node.tag || 'div';
          var t = tag.toLowerCase();
          var isHtml = (t === 'div' || t === 'span' || t === 'p' ||
                        t === 'button' || t === 'input' || t === 'ul' ||
                        t === 'li' || t === 'ol' || t === 'header' ||
                        t === 'footer' || t === 'section' ||
                        t === 'article' || t === 'main' || t === 'nav' ||
                        t === 'a' || t === 'img' || t === 'label');
          var domTag = isHtml ? t : 'div';
          var el = document.createElement(domTag);
          if (node.id) {
            el.setAttribute('data-node-id', node.id);
          }
          if (node.componentPath) {
            el.setAttribute('data-component-path', node.componentPath);
          }
          if (node.tag) {
            el.setAttribute('data-render-tag', node.tag);
          }
          // RS-M11b's `data-component-kind` annotation lives on
          // element-tree entries, not the render-tree. We don't have
          // a per-node kind here, but the launcher's style table is
          // kind-aware on the way out — the inline style map already
          // carries the kind's visual signature. Copy any inferred
          // kind from the style map's payload.
          applyStyle(el, node.style || {});
          // Text content goes on leaves only — if the node has
          // children we don't paint the parent's text (matches the
          // GPUI/Freya `textContent` accessor that returns the
          // descendant string for the whole subtree).
          if (!node.children || node.children.length === 0) {
            ...
            for (var i = 0; i < node.children.length; i++) {
              el.appendChild(materialiseNode(node.children[i]));
            }
          }
          return el;
        }
        function handleRenderTree(manifest) {
          // Replace host's children atomically. Children that are
          // not the materialised root (e.g. the editor's overlay
          // siblings inserted by canvas_mount) stay where they are
          // because they live as siblings of `host`, not under it.
          while (host.firstChild) host.removeChild(host.firstChild);
          var rootEl = materialiseNode(manifest.root);
          host.appendChild(rootEl);
          host.__isonimRenderTreeManifest = manifest;
          host.__isonimRenderTreeRoot = rootEl;
          try {
            if (window.__isonimTestMode === true) {
              window.__isonimRenderTreeManifest = manifest;
              window.__isonimRenderTreeRendererId = manifest.rendererId;
              if (!Array.isArray(window.__isonimRenderTreeManifests)) {
                window.__isonimRenderTreeManifests = [];
              }
              window.__isonimRenderTreeManifests.push(manifest);
            }
          } catch (_) {}
        }
        function encodeI(jsonStr) {
        ...
        function handleM(bytes) {
          var view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
          var length = readU32LE(view, 1);
          var json = decodeUtf8(bytes.subarray(5, 5 + length));
          var node;
          try { node = JSON.parse(json); } catch (_) { return; }
          if (node && node.type === 'render-tree') {
            handleRenderTree(node);
          }
          // Element-tree (incl. hello, resize, hot-reload) still flows
          // through dispatchMeta so the editor's overlay manifest
          // cache stays current.
          try {
            if (window.__isonimTestMode === true && node && node.type === 'element-tree') {
              window.__isonimManifest = node;
        ...
        ws.addEventListener('message', function (e) {
          ...
          if (bytes.length === 0) return;
          var kind = String.fromCharCode(bytes[0]);
          // RS-M13b: rendererSurface=='tree' means F packets are not
          // emitted on the wire. We still discard them defensively
          // so an older launcher can't break the editor.
          if (kind === 'M') { handleM(bytes); }
        });
        // Map pointer events on the render-tree host into manifest
        // hit-test coords. The element-tree manifest is authoritative
        // (RS-M11b); its bounds are in F-packet pixel space (the same
        // surface dims the render-tree carries). We map clientX/Y to
        // surface px via the host's clientWidth ratio.
        function pointFromEvent(e) {
          var rect = host.getBoundingClientRect();
          if (rect.width <= 0 || rect.height <= 0) return null;
          var manifest = host.__isonimRenderTreeManifest;
          var surfaceW = manifest ? (manifest.root && manifest.root.bounds && manifest.root.bounds.w) || rect.width : rect.width;
          var surfaceH = manifest ? (manifest.root && manifest.root.bounds && manifest.root.bounds.h) || rect.height : rect.height;
          var sx = surfaceW / rect.width;
          var sy = surfaceH / rect.height;
        ...
        host.addEventListener('mouseleave', onMouseLeave);
        host.addEventListener('click', onClickEvt);
        host.addEventListener('dblclick', onDblClickEvt);
        ws.__isonimRtHandlers = {
          mousemove: onMouseMove,
          mouseleave: onMouseLeave,
          click: onClickEvt,
          dblclick: onDblClickEvt,
        };
        ws.__isonimRtHost = host;
        """, socket, """ = ws;
      })(""", host, ", ", bridgeUrl.cstring, ", ", rendererId.cstring,
       ", ", dispatchMeta, ", ", onClick, ", ", onHover,
       ", ", onDblClick, ", ", onWsOpen, """);
    """].}
    RenderTreeHandle(socket: socket, host: host,
                     rendererId: rendererId, url: bridgeUrl)

  proc isRenderTreeBridgeOpen*(handle: RenderTreeHandle): bool =
    if handle == nil or handle.socket == nil: return false
    var sock = handle.socket
    var ready = 0
    {.emit: ["""
  ...
  proc sendRenderTreeSelectStory*(handle: RenderTreeHandle;
                                  storyGroup, storyName, storyKind,
                                  storyId: string) =
    ## RS-M13b: forward a `select-story` I packet over the bridge's
    ## WebSocket. Body bytes are byte-identical to the F/M/I I-body
    ## (the launcher decodes both the same way).
    if handle == nil: return
    var sock = handle.socket
    let body = encodeSelectStoryBody(storyGroup, storyName, storyKind,
                                     storyId)
  ...
  proc sendRenderTreeApplyMutation*(handle: RenderTreeHandle;
                                    target, key, valueLiteral: string;
                                    scope: MutationScopeKind) =
    if handle == nil: return
    var sock = handle.socket
  ...
  proc detachRenderTreeClient*(handle: RenderTreeHandle) =
    if handle == nil: return
    var sock = handle.socket
    let host = handle.host
    {.emit: ["""
      (function (ws, host) {
        if (ws) {
          try {
            if (ws.__isonimRtHandlers && host) {
              for (var k in ws.__isonimRtHandlers) {
                host.removeEventListener(k, ws.__isonimRtHandlers[k]);
              }
              ws.__isonimRtHandlers = null;
            }
          } catch (_) {}
          try { ws.close(); } catch (_) {}
        }
        if (host) {
          try {
            while (host.firstChild) host.removeChild(host.firstChild);
            host.removeAttribute('data-render-tree-host');
            host.__isonimRenderTreeManifest = null;
            host.__isonimRenderTreeRoot = null;
          } catch (_) {}
        }
      })(""", sock, ", ", host, ");"].}
    handle.socket = nil
